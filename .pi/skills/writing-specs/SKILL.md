...
- **Pause for user context after scouting.** Before the focused spec interview continues,
  ask one concise question grounded in the files and patterns you just read: whether the user
  has additional context, constraints, desired direction, or corrections. Wait for an explicit
  response before proceeding; `no extra context` or an equivalent answer is acceptable. Do not
  use a generic questionnaire, and do not ask the user to paste secrets, credentials, or tokens
  into `.incant/` artifacts.
